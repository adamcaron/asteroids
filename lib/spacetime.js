...
function ST (canvas, context, game) {
    ...
    this.shields   = [];
    this.mines     = [];
    this.initialAsteroids(game.numberOfAsteroids);
};
        ...
        if (asteroidCollidesWith(laser, asteroid)) {
            ...
            if(asteroid.hit_count === 3) {
                ...
                if (asteroid.width > 30) {
                    spaceTime.asteroids.push(new Asteroid(asteroid.x, asteroid.y, asteroid.width / 2, asteroid.height / 2, spaceTime.context, spaceTime.game.asteroidVelocity));
                    spaceTime.asteroids.push(new Asteroid(asteroid.x + asteroid.width / 2, asteroid.y + asteroid.height / 2, asteroid.width / 2, asteroid.height / 2, spaceTime.context, spaceTime.game.asteroidVelocity));
                }
            }

...
function detectLaserCollision() {
    spaceTime.asteroids.forEach( function(asteroid) {
...
function detectShipCollision() {
    spaceTime.asteroids.forEach( function(asteroid) {
            if(spaceTime.ship.y <= asteroid.y + asteroid.height && spaceTime.ship.y >= asteroid.y) {
                if(spaceTime.ship.x <= asteroid.x + asteroid.width && spaceTime.ship.x >= asteroid.x){
                    spaceTime.ship = []; //mildly hacky, need to think of something better
                }
            }
        });
}
